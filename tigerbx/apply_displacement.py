...
def transform(image_path, warp_path, output_dir=None, GPU=False, interpolation='nearest'):
    ...
    template_nib = lib_tool.get_template(None)
    ...
    input_data, _ = lib_bx.pad_to_shape(input_data, (256, 256, 256))
    input_data, _ = lib_bx.crop_image(input_data, target_shape=(256, 256, 256))
    ...
    if rigid_matrix is not None:
        ...
        rigid = lib_bx.remove_padding(np.squeeze(output[0]), pad_width)
    ...
    if affine_matrix is not None:
        ...
        affined = lib_bx.remove_padding(np.squeeze(output[0]), pad_width)
        affined_nib = nib.Nifti1Image(affined,
                                      template_nib.affine, template_nib.header)
        fn = bx.save_nib(affined_nib, ftemplate, 'Af')
    if dense_warp is not None:
    ...
    if Fuse_dense_warp is not None:
        affined_exp = np.expand_dims(np.expand_dims(affined, axis=0), axis=1)
