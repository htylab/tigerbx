...
Metric implementations ported from tigerbench (https://github.com/htylab/tigerbench).
Accepts NIfTI file paths, nibabel images, or numpy arrays as input.
...
-----
    ...
    # segmentation
    scores = tigerbx.eval('gt.nii.gz', 'pred.nii.gz', 'dice', labels=[10, 11, 17])
    scores = tigerbx.eval('gt.nii.gz', 'pred.nii.gz', ['dice', 'hd95'], labels=[1, 2, 3])

    # reconstruction
    scores = tigerbx.eval('ref.nii.gz', 'pred.nii.gz', ['psnr', 'ssim', 'ncc'])
...
def eval(y_true, y_pred, metrics, **kwargs):
    ...
    Parameters
    ----------
    y_true, y_pred : str | nibabel image | numpy array
        Ground truth and prediction. Accepts NIfTI file paths, nibabel
        image objects, or numpy arrays.
    metrics : str | list[str]
        Metric name(s) to compute. Supported: 'dice', 'iou', 'hd95', 'asd',
        'mae', 'mse', 'psnr', 'ssim', 'ncc', 'mi', 'ksg_mi',
        'accuracy', 'precision', 'recall', 'f1'.
    **kwargs
        Forwarded to each metric function. Common kwargs:
        - labels : list[int] — labels to evaluate (segmentation)
        - ignore_background : bool — exclude label 0 (default True)
        - average : str — sklearn averaging mode (classification)
        - bins : int — histogram bins for mi()
        - k : int — neighbours for ksg_mi()
        - data_range : float — for psnr() and ssim()
        - voxel_spacing : list[float] — for hd95() and asd()

    Returns
    -------
    dict
        Flat dict of {metric_name: value}. Segmentation metrics that return
        per-label dicts are stored under nested keys: {'dice': {...}}.

    Examples
    --------
    >>> import tigerbx
    >>> tigerbx.eval('gt.nii.gz', 'pred.nii.gz', 'dice', labels=[10, 11, 17])
    {'dice': {'10': 0.91, '11': 0.89, '17': 0.94, 'mean': 0.913}}
    >>> tigerbx.eval('gt.nii.gz', 'pred.nii.gz', ['psnr', 'ncc'])
    {'psnr': 38.2, 'ncc': 0.97}
    """
